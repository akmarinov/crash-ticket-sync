...
- Azure Boards: `AZURE_DEVOPS_PAT`, `AZURE_TACTILE_PAT`, or `AZURE_DEVOPS_EXT_PAT`
...
```bash
npm run dev -- sync -c crash-ticket-sync.config.json --project kylee-ios --since-hours 24
```

Dry run:

```bash
npm run dev -- sync -c crash-ticket-sync.config.json --project kylee-ios --dry-run
```
