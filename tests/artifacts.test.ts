...
function crash(overrides: Partial<CrashRecord> = {}): CrashRecord {
  return { projectKey: "kylee-ios", issueId: "abc123", title: "Crash", ...overrides };
}
...
describe("formatStacktrace", () => {
  it("renders header, exception line, and frames", () => {
    const text = formatStacktrace(crash(), event({
      ...
      frames: [
        { index: 0, library: "Kylee", symbol: "BluetoothManager.rescan()", file: "BluetoothManager.swift", line: 42, offset: 16, address: "0x1b4648" }
      ]
    }));
...
describe("buildAttachments", () => {
  it("emits stacktrace, events log, and recent-events for a detailed event", () => {
    const events = [event({
      frames: [{ index: 0, library: "Kylee", symbol: "f()", address: "0x1" }],
