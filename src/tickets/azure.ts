...
`
  ...
  private authHeader(): string {
    const token = process.env.AZURE_DEVOPS_PAT ?? process.env.AZURE_TACTILE_PAT ?? process.env.AZURE_DEVOPS_EXT_PAT;
    if (!token) throw new Error("Missing AZURE_DEVOPS_PAT, AZURE_TACTILE_PAT, or AZURE_DEVOPS_EXT_PAT");
